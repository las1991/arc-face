...
if [ -d "${CLASSPATH_LOCATION}" ]; then
  for file in "${CLASSPATH_LOCATION}"/*.jar; do
...
if [ -z "$JMEDIA_LOG4J_OPTS" ]; then
  #JMEDIA_LOG4J_OPTS="-Dlog4j.configuration=file:${APP_HOME}/config/log4j.properties"
  JMEDIA_LOG4J_OPTS="-Dlogback.configurationFile=file:${APP_HOME}/config/logback.xml -Dlogging.config=file:${APP_HOME}/config/logback.xml"
fi
